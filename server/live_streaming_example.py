...
strNaoIp = "169.254.168.203"
strNaoIp = "10.0.160.147"
strNaoIp = "10.9.45.11"
...
if( bUseMultiStreamModule and bStereoMode ):
    anVFlipCamera = [0, 0]; # stereo settings (was [0,1])
...
def findRedAndBlueMarks( imageBuffer, imageBufferForDrawingDebug = None, _nDetectionMethod = 0 ):
    ...
    #~ cv.SetImageCOI(  imageBuffer, 1 ); # a retester: c'est peut etre plus optimal (non: plein de m̩thode ne marche pas avec les COI)
    ...
    if( 1 ):
        ...
        if( dataImage == None ):
            ...
            if( bSaveFrame ):
                ...
                if( not bRecordMode ): led.fade( "AllLeds", 1., 0.2 );
                cv.SaveImage( strFilename, bufferImageToDraw );
                if( not bRecordMode ): led.fade( "AllLeds", 0., 0.1 );
                bSaveFrame = False;
                nCptImageSaved += 1;
            if( bChangeCamera ):
